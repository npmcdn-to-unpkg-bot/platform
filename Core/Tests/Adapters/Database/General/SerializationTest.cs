...
namespace Allors1.Adapters.General
{
    ...
    public abstract class SerializationTest : Test
    {
        ...
        private static void AssertSchema(string xml)
        {
            ...
            Assert.IsNotNull(allors);
            Assert.AreEqual("allors", allors.Name);

            var population = (XmlElement)allors.SelectSingleNode("*[1]");
            Assert.IsNotNull(population);
            Assert.AreEqual("population", population.Name);
            Assert.AreEqual("1", population.GetAttribute("version"));
            Assert.AreEqual(1, allors.SelectNodes("population").Count);

            var objects = (XmlElement)population.SelectSingleNode("*[1]");
            Assert.IsNotNull(objects);
            Assert.AreEqual("objects", objects.Name);
            Assert.AreEqual(1, population.SelectNodes("objects").Count);

            foreach (XmlElement ot in objects.SelectNodes("*/*"))
            {
                Assert.AreEqual("ot", ot.Name);
                Assert.AreEqual(0, ot.SelectNodes("*").Count);
                Assert.IsTrue(ot.InnerText.Length > 0);
            ...
            Assert.AreEqual("relations", relations.Name);
            Assert.AreEqual(1, population.SelectNodes("relations").Count);

            foreach (XmlElement rtx in relations.SelectNodes("*/*"))
            {
                Assert.IsTrue(rtx.Name.Equals("rtu") || rtx.Name.Equals("rtc"));
                Assert.IsTrue(rtx.SelectNodes("*").Count > 0);
                Assert.AreNotEqual(Guid.Empty, new Guid(rtx.GetAttribute("i")));

                foreach (XmlElement r in rtx.SelectNodes("*"))
                {
                    Assert.AreEqual("r", r.Name);
                    Assert.AreEqual(0, r.SelectNodes("*").Count);

                    if (rtx.Name.Equals("rtc"))
                    {
                        Assert.IsTrue(r.InnerText.Length > 0);
                    }
                }
            }
        }
