        ...
        public string GetSql(AssociationType associationType)
        {
            if (!this.sqlByAssociationType.ContainsKey(associationType))
            {
                ...
                if (associationType.RelationType.ExistExclusiveRootClasses)
                {
                    if (roleType.IsOne)
                    {
                        sql = Schema.AllorsPrefix + "GA_" + associationType.ObjectType.ExclusiveRootClass.Name + "_" + associationType.Name;
                    }
                    else
                    {
                        sql = Schema.AllorsPrefix + "GA_" + roleType.ObjectType.ExclusiveRootClass.Name + "_" + associationType.Name;
                    }
                }
                else
                {
        ...
        private class GetCompositeAssociation : DatabaseCommand, IGetCompositeAssociation
        {
            ...
            public Reference Execute(Reference role, AssociationType associationType)
            {
                ...
                if (result != null && result != DBNull.Value)
                {
                    ...
                    if (associationType.ObjectType.RootClasses.Count == 1 && associationType.ObjectType.ExclusiveRootClass.DerivedExclusiveConcreteLeafClass != null)
                    {
