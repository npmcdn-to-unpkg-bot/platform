...
namespace Allors.Domain
{
    ...
    {
        [Test]
        public void GivenSalesRepPartyRevenue_WhenDeriving_ThenDisplayNameIsSet()
        {
            ...
            var revenue = new SalesRepPartyRevenues(this.DatabaseSession).Extent().First;

            Assert.AreEqual(string.Format("{0}, {1}: {2}/{3} {4} at {5}", salesRep.DisplayName, customer.DisplayName, DateTime.Now.Year, DateTime.Now.Month, revenue.Revenue.AsCurrencyString(internalOrganisation.CurrencyFormat), internalOrganisation.DisplayName), revenue.DisplayName);
        }
