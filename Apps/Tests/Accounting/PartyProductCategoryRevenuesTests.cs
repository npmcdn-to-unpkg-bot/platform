...
namespace Allors.Domain
{
    ...
    {
        [Test]
        public void GivenPartyProductCategoryRevenue_WhenDeriving_ThenDisplayNameIsSet()
        {
            ...
            var revenue = new PartyProductCategoryRevenues(this.DatabaseSession).Extent().First;

            Assert.AreEqual(string.Format("{0}, {1}: {2}/{3} {4} at {5}", customer.DisplayName, cat.DisplayName, DateTime.Now.Year, DateTime.Now.Month, revenue.Revenue.AsCurrencyString(internalOrganisation.CurrencyFormat), internalOrganisation.DisplayName), revenue.DisplayName);
        }
