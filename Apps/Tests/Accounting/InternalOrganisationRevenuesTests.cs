...
namespace Allors.Domain
{
    ...
    {
        [Test]
        public void GivenInternalOrganisationRevenue_WhenDeriving_ThenDisplayNameIsSet()
        {
            ...
            var revenue = new InternalOrganisationRevenues(this.DatabaseSession).Extent().First;

            Assert.AreEqual(string.Format("{0}: {1}/{2} {3}", internalOrganisation.DisplayName, DateTime.Now.Year, DateTime.Now.Month, revenue.Revenue.AsCurrencyString(internalOrganisation.CurrencyFormat)), revenue.DisplayName);
        }
