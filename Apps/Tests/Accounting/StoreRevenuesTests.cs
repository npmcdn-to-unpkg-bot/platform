...
namespace Allors.Domain
{
    ...
    public class StoreRevenuesTests : DomainTest
    {
        ...
        public void GivenStoreRevenueHistory_WhenDeriving_ThenDisplayNameIsSet()
        {
            ...
            var store = internalOrganisation.StoresWhereOwner.First;
            ...
            var revenue = new StoreRevenues(this.DatabaseSession).Extent().First;

            Assert.AreEqual(string.Format("{0}: {1}/{2} {3} at {4}", store.DisplayName, DateTime.Now.Year, DateTime.Now.Month, revenue.Revenue.AsCurrencyString(internalOrganisation.CurrencyFormat), internalOrganisation.DisplayName), revenue.DisplayName);
        }
