...
namespace Allors.Domain
{
    ...
    public partial class PackageRevenueHistories
    {
        public static void AppsDeriveHistory(ISession session)
        {
            ...
            {
                var months = ((DateTime.Now.Year - packageRevenue.Year) * 12) + DateTime.Now.Month - packageRevenue.Month;
                if (months <= 12)
                {
            ...
            foreach (var keyValuePair in packageRevenuesByPeriodByPackageByInternalOrganisation)
            {
                ...
                foreach (var valuePair in keyValuePair.Value)
                {
                    ...
                    packageRevenueHistory.Derive().WithDerivation(derivation).Execute();
                }
            }
        }
