...
namespace Allors.Domain
{
    ...
    public partial class StoreRevenueHistories
    {
        public static void AppsDeriveHistory(ISession session)
        {
            ...
            {
                var months = ((DateTime.Now.Year - storeRevenue.Year) * 12) + DateTime.Now.Month - storeRevenue.Month;
                if (months <= 12)
                {
                    var date = new DateTime(storeRevenue.Year, storeRevenue.Month, 01);
            ...
            foreach (var keyValuePair in storeRevenuesByPeriodByStoreByInternalOrganisation)
            {
                ...
                foreach (var valuePair in keyValuePair.Value)
                {
                    ...
                    storeRevenueHistory.Derive().WithDerivation(derivation).Execute();
                }
            }
        }
