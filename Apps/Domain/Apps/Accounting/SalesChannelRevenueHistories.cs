...
namespace Allors.Domain
{
    ...
    public partial class SalesChannelRevenueHistories
    {
        public static void AppsDeriveHistory(ISession session)
        {
            ...
            {
                var months = ((DateTime.Now.Year - salesChannelRevenue.Year) * 12) + DateTime.Now.Month - salesChannelRevenue.Month;
                if (months <= 12)
                {
                    var date = new DateTime(salesChannelRevenue.Year, salesChannelRevenue.Month, 01);
            ...
            foreach (var keyValuePair in salesChannelRevenuesByPeriodBySalesChannelByInternalOrganisation)
            {
                ...
                foreach (var valuePair in keyValuePair.Value)
                {
                    ...
                    salesChannelRevenueHistory.Derive().WithDerivation(derivation).Execute();
                }
            }
        }
