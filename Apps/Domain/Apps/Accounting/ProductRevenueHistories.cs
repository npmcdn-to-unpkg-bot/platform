...
namespace Allors.Domain
{
    ...
    public partial class ProductRevenueHistories
    {
        public static void AppsDeriveHistory(ISession session)
        {
            ...
            {
                var months = ((DateTime.Now.Year - productRevenue.Year) * 12) + DateTime.Now.Month - productRevenue.Month;
                if (months <= 12)
                {
            ...
            foreach (var keyValuePair in productRevenuesByPeriodByProductByInternalOrganisation)
            {
                ...
                foreach (var valuePair in keyValuePair.Value)
                {
                    ...
                    productRevenueHistory.Derive().WithDerivation(derivation).Execute();
                }
            }
        }
