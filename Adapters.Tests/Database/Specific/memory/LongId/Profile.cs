...
namespace Allors.Adapters.Special.Memory.LongId
{
    ...
    public class Profile : Special.Profile
    {
        ...
        {
            return new Database.Memory.LongId.Database(new Database.Memory.LongId.Configuration { ObjectFactory = this.ObjectFactory });
        }

        public override IWorkspace CreateWorkspace(IDatabase database)
        {
            var configuration = new Workspace.Memory.LongId.Configuration { Database = database };
